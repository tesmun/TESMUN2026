...
import principalSpeech from "@/content/principal-speech.txt?raw";
...
export default function PressSection() {
  ...
  {slug === "interviews" && <section className="mt-14 grid gap-7 lg:grid-cols-2">{interviewSubjects.map((item) => <article key={item.title} className="border border-white/10 bg-navy p-5 shadow-2xl">{item.transcript ? <Link to="/press/interviews/episode-15" className="group block"><div className="mb-5 aspect-video overflow-hidden bg-navy-deep"><img src="/images/press/interview-15.png" alt="Episode 15 interview with Sheetal Rana" className="h-full w-full object-cover transition group-hover:opacity-90" /></div></Link> : <div className="mb-5 aspect-video bg-navy-deep"><YouTubeEmbed url={item.url} title={`${item.title} — ${item.subject}`}/></div>}<p className="text-[10px] uppercase tracking-[0.16em] text-gold">Interview</p><h2 className="mt-2 font-display text-3xl">{item.title} — {item.subject}</h2></article>)}</section>}
