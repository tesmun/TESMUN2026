...
import secretaryGeneralSpeech from "@/content/secretary-general-speech.txt?raw";
import disecChairSpeech from "@/content/disec-chair-speech.txt?raw";
import hrcChairSpeech from "@/content/hrc-chair-speech.txt?raw";
import itHeadSpeech from "@/content/it-head-speech.txt?raw";
import pressHeadSpeech from "@/content/press-head-speech.txt?raw";
import logisticsHeadSpeech from "@/content/logistics-head-speech.txt?raw";
import openingClosingSpeech from "@/content/opening-closing-speech.txt?raw";
...
const speechRoster = [{ speaker: "Dibas Khadka", role: "Secretary General", committee: "Opening Ceremony", photo: "/images/our-team/secgen.png", transcript: openingClosingSpeech }, { speaker: "Abhash Kunwar", role: "Deputy Secretary General", committee: "Opening Ceremony", photo: "/images/our-team/abhas.png" }, ...["LP I", "LP II", "LP III", "UNEP", "DISEC", "ECOSOC", "HRC"].map((committee) => ({ speaker: `${committee} Chair`, role: "Committee Chair", committee, photo: chairPhotos[committee], transcript: committee === "DISEC" ? disecChairSpeech : committee === "HRC" ? hrcChairSpeech : undefined })), { speaker: "Head of IT", role: "Head of IT", committee: "Information Technology", photo: "/images/our-team/it/yubin.png", transcript: itHeadSpeech }, { speaker: "Dechen Hira Tamang", role: "Head of Press", committee: "Press", photo: "/images/our-team/press/dechen.png", transcript: pressHeadSpeech }, { speaker: "Prasina Moktan", role: "Head of Logistics (Girl)", committee: "Logistics", photo: "/images/our-team/logistics/prasina.png", transcript: logisticsHeadSpeech }];
...
export default function PressSection() {
  ...
  {slug === "speeches" && <section className="mt-14 max-w-5xl divide-y divide-white/15 border-y border-white/15">{speechRoster.map((speech, i) => <div key={speech.speaker}><button type="button" onClick={() => setOpen(open === i ? -1 : i)} className="flex w-full items-center justify-between gap-5 py-8 text-left"><span className="flex items-center gap-6"><img src={speech.photo} alt={`${speech.speaker} portrait`} className="h-24 w-24 shrink-0 rounded-full border border-gold/30 object-cover sm:h-32 sm:w-32" /><span><span className="text-[10px] uppercase tracking-[0.16em] text-gold">{speech.committee} · {speech.role}</span><span className="mt-2 block font-display text-3xl sm:text-4xl">{speech.speaker}</span></span></span><ChevronDown className={`shrink-0 transition ${open === i ? "rotate-180 text-gold" : ""}`} /></button>{open === i && <div className="whitespace-pre-line pb-8 pl-0 text-base leading-8 text-silver/80 sm:pl-36">{(speech as { transcript?: string }).transcript || "This speech transcript will appear here in the conference archive."}</div>}</div>)}</section>}
