...
const podcastEpisodes = ["https://youtu.be/CwUKSr0yKv0", video];
const voxTeamOne = [
...
const interviewSubjects = [
  ...
  { title: "EPISODE 09", subject: "Interview", url: "https://www.youtube.com/watch?v=MIswbwENbYM" },
  { title: "EPISODE 10", subject: "Interview", url: "https://www.youtube.com/watch?v=WT9LbKX2PFI" },
  { title: "EPISODE 11", subject: "Interview", url: "https://www.youtube.com/watch?v=Unr_HnUKVcc" },
];
...
export default function PressSection() {
  ...
  {slug === "podcast" && <section className="mt-14 grid max-w-4xl gap-6 sm:grid-cols-2">{podcastEpisodes.map((episodeUrl, index) => { const title = `Podcast episode ${String(index + 1).padStart(2, "0")}`; return <article key={episodeUrl} className="border border-white/10 bg-navy p-5"><div className="aspect-video bg-navy-deep"><YouTubeEmbed url={episodeUrl} title={title}/></div><h2 className="mt-4 font-display text-3xl">{title}</h2></article>; })}</section>}
