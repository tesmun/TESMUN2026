...
function ArticleStory({ article }: { article: SessionNewsArticle }) {
  return <Link to={`/press/news/${article.slug}`} className="group block border-b border-[#171717]/25 py-10 first:pt-0 last:border-b-0 sm:py-14 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#171717]"><div className="grid gap-7 lg:grid-cols-[1.2fr_0.9fr] lg:items-start lg:gap-12"><figure className="overflow-hidden bg-[#171717]"><img src={article.image} alt={`${article.committee} editorial photograph`} className="aspect-[16/10] w-full object-cover grayscale contrast-110 saturate-0 transition duration-700 hover:scale-[1.035]" /><figcaption className="bg-[#171717] px-3 py-2 font-mono text-[9px] uppercase tracking-[0.14em] text-[#f5f0e7]/70">Field image · {article.committee}</figcaption></figure><div><div className="flex items-center gap-3 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-[#171717]/60"><span>Report {article.articleNumber}</span><span>·</span><span>{article.date}</span></div><div className="mt-4 flex items-center gap-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-[#171717]/70">{article.committeeLogo && <img src={article.committeeLogo} alt="" className="size-7 rounded-full border border-[#171717]/40 bg-[#f5f0e7] object-contain p-1 grayscale" />}{article.committee}</div><h2 className="mt-4 font-display text-4xl leading-[0.9] tracking-[-0.04em] text-[#171717] sm:text-5xl">{article.title}</h2><p className="mt-5 font-serif text-lg leading-7 text-[#171717]/75">{article.standfirst}</p><div className="mt-5 space-y-4 font-serif text-[15px] leading-7 text-[#171717]/70">{article.body.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><p className="mt-6 font-mono text-[9px] font-bold uppercase tracking-[0.16em] text-[#171717]/50">By {article.author}</p></div></div></Link>;
}
