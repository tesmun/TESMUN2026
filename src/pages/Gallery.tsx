...
const photos = (offset: number, count: number, custom?: string[], alt = "Gallery photograph") => custom ? Array.from({ length: count }, (_, index) => ({ src: custom[index % custom.length], alt })) : Array.from({ length: count }, (_, index) => galleryImages[(offset + index) % galleryImages.length]);
const sessions = [
